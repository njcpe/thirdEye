...
public class VideoQuality {
	...
	public final static VideoQuality DEFAULT_VIDEO_QUALITY = new VideoQuality(176,144,15,500000);
	...
	public boolean equals(VideoQuality quality) {
		return quality != null && (quality.resX == this.resX && quality.resY == this.resY && quality.framerate == this.framerate && quality.bitrate == this.bitrate);
	}
	...
	public static VideoQuality determineClosestSupportedResolution(Camera.Parameters parameters, VideoQuality quality) {
		...
		for (Iterator<Size> it = supportedSizes.iterator(); it.hasNext();) {
			...
			if (dist<minDist || distY < minDistY) {
				minDist = dist;
				v.resX = size.width;
				v.resY = size.height;
				minDistY = distY;
		...
		Log.e(TAG, supportedSizesStr.toString());
		if (quality.resX != v.resX || quality.resY != v.resY) {
			Log.e(TAG,"Resolution modified: "+quality.resX+"x"+quality.resY+"->"+v.resX+"x"+v.resY);
		}
	...
	public static int[] determineMaximumSupportedFramerate(Camera.Parameters parameters) {
		...
		Log.d(TAG, "supportedFpsRangesStr: " + supportedFpsRangesStr);
